...
abstract class BaseDao
{
    ...
    /**
     * chooseTableName [设置最新表名并返回最新uid]
     * @param null $id
     * @return mixed
     */
    public function chooseTableName($id = null){
        return $this->getModel()->setTableName($id);
    }

    /**
     * setTableNameByName [设置成指定表名]
     * @param $table_name
     */
    public function setTableNameByName($table_name){
        $this->getModel()->setTabelNameByName($table_name);
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getForeach($id, ?array $field = [], ?array $with = [])
    {
        if (is_array($id)) {
            $where = $id;
        } else {
        ...
        $suffix = $table_name_arr[1];
        for ($i = $suffix; $i >= 1; $i--){
            $this->setTableNameByName($table_name_arr[0].'_'.$i);
            $res = $this->getModel()->where($where)->when(count($with), function ($query) use ($with) {
                $query->with($with);
            })->field($field ?? ['*'])->find();
            if ($res){
                $res->setTabelNameByName($table_name_arr[0].'_'.$i);
                break;
            }
        }
        return $res;
...
//        })->field($field ?? ['*'])->find();
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function getOneForeach(array $where, ?string $field = '*',array $with = []){
        $field = explode(',', $field);
        return $this->getForeach($where, $field, $with);
    }
     ...
     * @throws \think\db\exception\DbException
     */
    public function updateByTable($id, array $data, ?string $key = null,$table_name = null){
        if (!$table_name){
            return false;
        }
        if (is_array($id)) {
        ...
        return Db::name($table_name)->where($where)->update($data);
    }
